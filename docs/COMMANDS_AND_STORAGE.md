...
上述恢复协议已落地为 `DocumentRecoveryService` + `CadRecoveryStore`：30 秒定时、两代完整快照、进程排他锁、损坏回退、正常关闭清理，以及恢复窗口的“恢复为未保存副本”。恢复交接先建立新持久副本再退休来源；详细磁盘结构和验收见 [RECOVERY.md](RECOVERY.md)。
