...
保持已有根目录下 `Cadoryx.*` 命名方式。以下模块已创建，恢复快照与会话入口已接通；表中预算、完整编辑器命令等扩展职责仍需按路线逐项实施。
...
- 独立应用弹窗统一 `mah:MetroWindow`，共用 `Cadoryx.DialogWindowStyle`；owner 与居中位置由各窗口设置。文档资源窗口包含零件/图层/材料，实例局部位置放在属性面板，详见 [资源与实例](RESOURCES_AND_INSTANCES.md)。
- 关闭文档通过 `CadDocumentViewModel.Detaching` 显式释放 native 视口，再释放会话租约；WPF Unloaded 只负责布局卸载，不能作为唯一的关闭释放信号。
